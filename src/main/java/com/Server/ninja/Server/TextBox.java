...
public class TextBox
{
    protected static void openTextBoxId(final Char _char, final short menuId, final String str, byte type) {
        ...
        switch (menuId) {
            ...
            case 997: {
                if (!Util.CheckString(str, "^[a-zA-Z0-9]+$") || str.length() < 6 || str.length() > 15) {
                    GameCanvas.startOKDlg(_char.user.session, "Tài khoản chỉ đồng ý các ký tự a-z,0-9 và chiều dài từ 6 đến 15 ký tự");
                } else {
                    _char.user.player.resigerUname = str.toLowerCase();
                    Service.openTextBoxUI(_char, "Nhập mật khẩu đăng ký", (short)996);
                }
                break;
            }
            case  996: {
                if (!Util.CheckString(str, "^[a-zA-Z0-9]+$") || str.length() < 6 || str.length() > 15) {
                    GameCanvas.startOKDlg(_char.user.session, "Mật khẩu chỉ đồng ý các ký tự a-z,0-9 và chiều dài từ 6 đến 15 ký tự");
                } else {
                    _char.user.player.resigerPass = str.toLowerCase();
                    try {
                        MySQL mySQL = new MySQL(1);
                        ResultSet red = mySQL.stat.executeQuery("SELECT `user` FROM `user` WHERE `user` LIKE '" + Util.strSQL(_char.user.player.resigerUname) + "' LIMIT 1;");
                        if (red == null || !red.first()) {
                            mySQL.stat.executeUpdate("UPDATE `user` SET `user`='" + Util.strSQL(_char.user.player.resigerUname) + "',`password`= '" + Util.strSQL(_char.user.player.resigerPass) + "', `randomId` = 0, `tester` = 0, `status` = 0 WHERE `userId`=" + _char.user.userId + " LIMIT 1;");
                            GameCanvas.startOKDlg(_char.user.session, "Đăng ký thành công tài khoản " + _char.user.player.resigerUname + " mật khẩu " + _char.user.player.resigerPass);
                        } else {
                            GameCanvas.startOKDlg(_char.user.session, "Tài khoản đã tồn tại. Vui lòng thử lại.");
                        }
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
