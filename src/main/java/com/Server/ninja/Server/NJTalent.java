...
public class NJTalent {
    ...
    protected static void close() {
        synchronized (NJTalent.LOCK) {
            if (NJTalent.ninja_talent != null) {
                NJTalent.ninja_talent.CLOSE();
                NJTalent.ninja_talent = null;
                NJTalent.isRun = false;
            }
        }
    }
    
    protected void CLOSE() {
        final Char[] arrChar = new Char[this.memjoin.size()];
        synchronized (this.memjoin) {
            for (byte i = 0; i < arrChar.length; ++i) {
                arrChar[i] = this.memjoin.get(i);
            }
        }
