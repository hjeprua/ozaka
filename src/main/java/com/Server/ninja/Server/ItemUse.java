        ...
        if (item != null) {
            if (item.template.level > _char.cLevel) {
                ...
                if (!_char.isHuman) {
                    ...
                    short idItem = 899;
                    if (Util.nextInt(120) == 7) {
                        idItem = 902;
                    }
                    if (Util.nextInt(120) < 5) {
                        idItem = 901;
                    }
                    ...
                    if (Util.nextInt(120) < 3) {
                        itemUp.expires = -1;
                        Client.chatServer("Admin ", String.format("Chúc mừng %s đã phá phong ấn nhận được %s vĩnh viễn", _char.cName, itemUp.template.name));
                    }
                    _char.user.player.ItemBagAdd(itemUp);
                    _char.user.player.ItemBagUse(indexUI, 1);
                }
